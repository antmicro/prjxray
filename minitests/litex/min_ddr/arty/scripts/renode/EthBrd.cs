...
namespace Antmicro.Renode.Peripherals
{
    ...
    public class EthBrd : IDoubleWordPeripheral, IDisposable, IAbsoluteAddressAware
    {
        public EthBrd(int port, string host = "127.0.0.1")
        {
            try
            {
		...
		this.Log(LogLevel.Info, "Reading from address 0x{0:X}", offset);
...
this.Log(LogLevel.Error, "Reading error");
                result = 0;
                return false;
            }
...
this.Log(LogLevel.Info, "Read from address 0x{0:X} returned 0x{1:X}", offset, result);
		...
		this.Log(LogLevel.Info, "Writing 0x{1:X} to address 0x{0:X}", offset, val);
            try
            {
                dataStream.Write(bytes, 0, bytes.Length);
		this.Log(LogLevel.Info, "Write ok");
                return true;
            }
            catch(IOException)
            {
		this.Log(LogLevel.Error, "Write error");
                return false;
            }
        }
        ...
        private void FixFieldEndianess(byte[] arr)
        {
            ...
            SwapElements(arr, 12, 15);
            SwapElements(arr, 13, 14);

            // swap WriteValueReadAddress
            SwapElements(arr, 16, 19);
            SwapElements(arr, 17, 18);

        }

            private void SwapElements(byte[] arr, int id1, int id2)
            {
                byte tmp;
